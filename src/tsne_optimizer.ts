...
export class TSNEOptimizer {
  ...
  get numberOfPointsPerRow() {
    return this.pointsPerRow
  }
  get numberOfRows() {
    return this.numRows
  }
  get embeddingCoordinates() {
  ...
  get embedding2D(): tf.Tensor {
    const result = tf.tidy(() => {
      ...
      const reshaped = this.embedding
        .reshape([this.numRows * this.pointsPerRow, 2])
  ...
  set exaggeration(ex: number | Array<{ iteration: number; value: number }>) {
    ...
    if (typeof ex === "number") {
      ...
      for (let i = 0; i < ex.length; ++i) {
        ...
        if (ex[i].iteration < 0) {
          throw Error(
            "Piecewise linear exaggeration function \
  ...
  get momentum(): number {
    return this._momentum.arraySync()
  }
  ...
  constructor(
    numPoints: number,
    verbose?: boolean,
    splatTextureDiameter?: number,
    kernelTextureRadius?: number
  ) {
    if (verbose != null) {
    ...
    this.rawExaggeration = [
      { iteration: 200, value: 4 },
      { iteration: 600, value: 1 },
    ]
    this.updateExaggeration()
  ...
  initializeEmbedding() {
    ...
    this.embedding = tf.tidy(() => {
      const randomData = tf.randomUniform([this.numRows, this.pointsPerRow * 2])
  ...
  initializeNeighbors(
    numNeighPerRow: number,
    offsets: WebGLTexture,
    probabilities: WebGLTexture,
    neighIds: WebGLTexture
  ) {
  ...
  async initializeNeighborsFromKNNGraph(
    numPoints: number,
    numNeighbors: number,
  ...
  // Defines the neighborhood relationships between the points
  async initializeNeighborsFromKNNTexture(
    ...
    if (shape.numPoints !== this.numPoints) {
      throw new Error(
        `KNN size and number of points must agree` +
    ...
    const gaussianDistributions = tf.zeros([
      shape.numRows,
      shape.pointsPerRow * shape.pixelsPerPoint,
    ])

    const perplexity = shape.pixelsPerPoint / 3
    // Computation of the per-point probability vectors
    this.computeDistributionParameters(
      distributionParameters,
      shape,
      perplexity,
      knnGraph
    )
    this.computeGaussianDistributions(
    ...
    const asymNeighIds = new Float32Array(
      shape.numPoints * shape.pixelsPerPoint
    )
    ...
    for (let i = 1; i < shape.numPoints; ++i) {
      neighborLinearOffset[i] =
        neighborLinearOffset[i - 1] +
        neighborCounter[i - 1] +
    ...
    this.numNeighPerRow = Math.ceil(
      Math.sqrt(shape.numPoints * shape.pixelsPerPoint * 2)
    )
    ...
    // Probabilities && Indices
    {
      const probabilities = new Float32Array(
        this.numNeighPerRow * this.numNeighPerRow
      )
      const neighIds = new Float32Array(
        this.numNeighPerRow * this.numNeighPerRow
      )
  ...
  async iterate(): Promise<void> {
    if (!this.initializedNeighborhoods()) {
      throw new Error(
        "No neighborhoods defined. You may want to call\
    ...
    ;[this.gradient, normQ] = tf.tidy(() => {
      ...
      const gradientIter = attractiveForces
        .mul(this._exaggeration)
        .sub(repulsiveForces)
  ...
  private initializeRepulsiveForceTextures() {
    ...
    const kernel = new Float32Array(
      this.kernelTextureDiameter * this.kernelTextureDiameter * 4
    )
  ...
  private initilizeCustomWebGLPrograms() {
    ...
    this.embeddingInitializationProgram = tsne_util.createEmbeddingInitializationProgram(
      this.gpgpu
    )

    this.embeddingSplatterProgram = tsne_util.createEmbeddingSplatterProgram(
      this.gpgpu
    )
    ...
    this.xyInterpolatorProgram = tsne_util.createXYInterpolatorProgram(
      this.gpgpu
    )

    // Computation of the attracive forces
    this.attractiveForcesProgram = tsne_util.createAttractiveForcesComputationProgram(
      this.gpgpu
    )

    // Computation of the joint probability distribution
    this.distributionParameterssComputationProgram = tsne_util.createDistributionParametersComputationProgram(
      this.gpgpu
    )
    this.gaussiaDistributionsFromDistancesProgram = tsne_util.createGaussiaDistributionsFromDistancesProgram(
      this.gpgpu
    )
  }
  ...
  private async computeBoundaries(): Promise<void> {
    const [min, max] = tf.tidy(() => {
      // 2d tensor with some extra points
      const embedding2D = this.embedding
        .reshape([this.numRows * this.pointsPerRow, 2])
        .slice([0, 0], [this.numPoints, 2])
  ...
  private updateSplatTextureDiameter() {
    ...
    const textureDiameter = Math.min(
      Math.ceil(Math.max(maxSpace / spacePerPixel, 5)),
      maxTextureDiameter
    )

    const percChange =
      Math.abs(this.splatTextureDiameter - textureDiameter) /
      this.splatTextureDiameter
  ...
  private initializeEmbeddingPositions(
    ...
    tsne_util.executeEmbeddingInitializationProgram(
      ...
      this.backend.getTexture(random.dataId),
      this.numPoints,
      this.pointsPerRow,
      this.numRows,
  ...
  private computeInterpolatedQ(interpolatedQ: tf.Tensor) {
    tsne_util.executeQInterpolatorProgram(
      ...
      this.pointsPerRow,
      this.numRows,
      this.backend.getTexture(interpolatedQ.dataId)
    )
  }
  ...
  private computeInterpolatedXY(interpolatedXY: tf.Tensor) {
    tsne_util.executeXYInterpolatorProgram(
      ...
      this._splatTexture,
      this.backend.getTexture(this.embedding.dataId),
      this.backend.getTexture(interpolatedXY.dataId),
      this.numPoints,
      this._minX,
      this._minY,
      this._maxX,
  ...
  private computeAttractiveForces(attractiveForces: tf.Tensor) {
    tsne_util.executeAttractiveForcesComputationProgram(
      ...
      this.backend.getTexture(this.embedding.dataId),
      this.probOffsetTexture,
      this.probNeighIdTexture,
      this.probTexture,
      this.numPoints,
      this.numNeighPerRow,
      this.pointsPerRow,
